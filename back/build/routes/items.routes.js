...
router.get('/items', async (_req, res) => {
    try {
      ...
      LIMIT 200;
    `;
        const r = await db_1.pool.query(sql);
        res.json(r.rows);
    }
    catch (err) {
        console.error(err);
...
router.post('/items', async (req, res) => {
    try {
        const schema = process.env.DB_SCHEMA || 'inventario';
        const { codigo_interno, nombre, modelo = null, descripcion = null, vida_util_meses = null, condicion_fisica = null, activo = true, id_subcategoria, id_marca = null, id_adquisicion = null, id_user_actual = null, id_area_actual = null, } = req.body;
    ...
    `;
        ...
        const insertResult = await db_1.pool.query(insertSql, insertValues);
      ...
      WHERE i.id_item = $1;
    `;
        const itemResult = await db_1.pool.query(selectSql, [id_item]);
